...
import type { Course, Lesson, LessonAttachment, Quiz } from "@/lib/db/types";
...
export default function LessonManagerPanel({ course }: { course: Course }) {
  ...
  // Lazily check whether an expanded lesson already has a quiz attached —
  // only fetched on demand (not for every lesson up front) so opening the
  // panel for a course with dozens of lessons doesn't fire a burst of
  // quiz queries nobody's about to look at.
  useEffect(() => {
    if (!openLessonId || openLessonId in quizByLesson) return;
    quizQueries
      .getByLessonId(openLessonId)
      .then(quiz => setQuizByLesson(prev => ({ ...prev, [openLessonId]: quiz })))
      .catch(() => setQuizByLesson(prev => ({ ...prev, [openLessonId]: null })));
  }, [openLessonId, quizByLesson]);

  // One cheap bulk query for the whole course so every collapsed lesson row
  // can show a "Quiz · N questions" / "No quiz yet" badge up front — the
  // per-lesson quiz status shouldn't be something a teacher only discovers
  // after opening each lesson individually.
  const [quizStatusByLesson, setQuizStatusByLesson] = useState<
    Record<string, { title: string; status: string; questionCount: number }>
  >({});
  useEffect(() => {
    quizQueries
      .getStatusByCourse(course.id)
      .then(setQuizStatusByLesson)
      .catch(() => setQuizStatusByLesson({}));
  ...
  return (
    <div className="flex flex-col gap-tight border-t border-line pt-stack">
      {lessons.length === 0 && !creating ? (
        ...
        <div className="flex flex-col gap-1">
          {visible.map((lesson, i) => {
            ...
            const quizStatus = quizStatusByLesson[lesson.id];
            return (
              // bg-mist turns this row into the same light "paper" island the
              // rest of the app uses for content-heavy surfaces (see
              // LessonContentCard / .lesson-card in index.css) — the text and
              // border tokens below (text-ink, text-slate, border-line) are
              // designed for that light surface, not the dark dashboard panel
              // this component is mounted inside. Without this, lesson titles
              // and the quiz button were dark-navy text on a near-black
              // background — unreadable.
              <div key={lesson.id} className="overflow-hidden rounded-control border border-line bg-mist">
                <div className="flex items-center gap-tight px-stack py-tight">
                  ...
                  >
                    <span className="flex flex-wrap items-center gap-1.5">
                      {i + 1}. {lesson.title}
                      {lesson.video_url ? <Video size={12} className="inline text-accent" aria-hidden="true" /> : null}
                      {quizStatus ? (
                        <span
                          className={`inline-flex items-center gap-1 rounded-pill px-tight py-0.5 text-[10px] uppercase tracking-widest ${
                            quizStatus.status === "published" ? "bg-success/15 text-success" : "bg-accent/15 text-accent"
                          }`}
                        >
                          <ClipboardList size={10} aria-hidden="true" />
                          {quizStatus.questionCount} {t("components.teacher.LessonManagerPanel.quizQuestions")}
                          {quizStatus.status === "published" ? "" : ` · ${t("components.teacher.LessonManagerPanel.quizDraftBadge")}`}
                        </span>
                      ) : null}
                    </span>
          ...
          {lessons.length > COLLAPSE_AFTER ? (
            <button type="button" onClick={() => setShowAll(v => !v)} className="w-fit self-center text-xs text-lavender underline">
      ...
      {creating ? (
        // Same light-card rationale as each lesson row above — the editor
        // pane's inputs/labels (text-ink, text-slate) need a light ambient
        // background; the ResizableSplit "right" preview pane still sets its
        // own bg-night to reproduce the real dark Learn page.
        <div className="flex flex-col gap-tight rounded-control border border-line bg-mist">
      ...
      {showFullPreview ? (
        <div className="fixed inset-0 z-50 flex flex-col overflow-hidden bg-night text-night-foreground">
          <div className="flex shrink-0 items-center justify-between gap-tight border-b border-hairline px-gutter py-tight">
            ...
            <button
              ...
              className="rounded-control border border-hairline px-stack py-tight text-xs text-lavender hover:text-night-foreground"
            >
              {t("components.teacher.LessonManagerPanel.cancel")}
            </button>
          </div>
      ...
      {quizBuilderLessonId ? (
        <QuizBuilder
          lesson={lessons.find(l => l.id === quizBuilderLessonId)!}
          onClose={() => setQuizBuilderLessonId(null)}
          onSaved={quiz => {
            setQuizByLesson(prev => ({ ...prev, [quiz.lesson_id]: quiz }));
            setQuizBuilderLessonId(null);
            quizQueries.getStatusByCourse(course.id).then(setQuizStatusByLesson).catch(() => {});
          }}
          onDeleted={() => {
            setQuizByLesson(prev => ({ ...prev, [quizBuilderLessonId]: null }));
            setQuizBuilderLessonId(null);
            quizQueries.getStatusByCourse(course.id).then(setQuizStatusByLesson).catch(() => {});
          }}
        />
