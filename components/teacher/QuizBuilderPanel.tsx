...
export default function QuizBuilderPanel({
  ...
  const uploadImage = async (file: File, target: { qIndex: number; oIndex?: number }) => {
    ...
    try {
      ...
      if (target.oIndex === undefined) updateQuestion(target.qIndex, { image_url: url });
      else updateOption(target.qIndex, target.oIndex, { image_url: url });
    } catch (err) {
      console.error("[QuizBuilderPanel] image upload error:", err);
  ...
  const updateOption = (
    qIndex: number,
    oIndex: number,
    patch: Partial<DraftQuestion["options"][number]>
  ) => {
    setQuestions((prev) =>
      prev.map((q, i) => {
        ...
        i !== qIndex
          ...
          : {
              ...
              options: q.options.map((o, j) =>
                j === oIndex ? { ...o, is_correct: !o.is_correct } : o
              ),
            }
      )
    );
  ...
  const handleSave = async () => {
    ...
    try {
      const cleaned = questions.map((q) => ({
        ...q,
        options: q.options.filter((o) => o.label.trim().length > 0 || o.image_url),
      }));
      ...
      console.error("[QuizBuilderPanel] save error:", err);
  ...
  const handleTogglePublish = async () => {
    ...
    try {
      ...
      console.error("[QuizBuilderPanel] status error:", err);
      setError(T("errorSave"));
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.error("[QuizBuilderPanel] delete error:", err);
      setError(T("errorSave"));
    } finally {
      setDeleting(false);
  ...
  return (
    <div className="card-lift flex flex-col overflow-hidden rounded-card border border-line bg-background">
      {/* ---------- Header ---------- */}
      <div className="flex flex-wrap items-center justify-between gap-stack border-b border-line px-block py-3">
        <div className="flex min-w-0 items-center gap-stack">
          <button
            type="button"
            onClick={onClose}
            className="inline-flex shrink-0 items-center gap-1.5 rounded-control border border-line px-3 py-1.5 text-xs text-slate transition-colors duration-base hover:border-accent hover:text-ink"
          >
            <ChevronLeft size={14} aria-hidden="true" />
            {T("cancel")}
          </button>
          <div className="flex min-w-0 items-center gap-tight">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-control bg-accent/10 text-accent">
              <Sparkles size={15} aria-hidden="true" />
            </span>
            <div className="min-w-0">
              <p className="truncate text-sm font-medium text-ink">{T("heading")}</p>
              <p className="truncate text-xs text-slate">{lesson.title}</p>
            </div>
          </div>
        </div>
        ...
        {existingQuizId ? (
          <div className="flex shrink-0 items-center gap-2">
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-control px-3 py-1.5 text-xs font-medium transition-colors duration-base disabled:opacity-60 ${
                status === "published"
            ...
            {confirmDelete ? (
              <div className="flex items-center gap-1">
                <button
                  ...
                  disabled={deleting}
                  className="inline-flex items-center gap-1.5 rounded-control bg-danger px-3 py-1.5 text-xs font-medium text-danger-foreground hover:opacity-90 disabled:opacity-60"
                >
                ...
                <button
                  ...
                  className="rounded-control border border-line px-3 py-1.5 text-xs text-slate hover:text-ink"
                >
                  Cancel
                </button>
              </div>
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-control border border-danger/40 px-3 py-1.5 text-xs font-medium text-danger transition-colors duration-base hover:bg-danger/10"
          ...
          <span className="shrink-0 text-xs text-slate">Unsaved</span>
        )}
      </div>

      {/* ---------- Body ---------- */}
      {loading ? (
        <div className="flex items-center justify-center py-block">
          <Loader2 size={22} className="animate-spin text-accent" aria-hidden="true" />
        </div>
      ) : (
        <div className="max-h-[70vh] overflow-y-auto px-block py-block">
            ...
            <div className="grid grid-cols-2 gap-tight sm:grid-cols-3">
              <label className="flex flex-col gap-1">
                <span className="flex items-center gap-1 text-xs font-medium text-slate">
                  <Target size={12} /> {T("passingScoreLabel")}
                </span>
                <input
                  type="number"
                  min={0}
                  ...
                  value={passingScore}
                  onChange={(e) =>
                    setPassingScore(Math.min(100, Math.max(0, Number(e.target.value))))
                  }
                  className="rounded-control border border-line bg-background px-stack py-tight text-sm text-ink outline-none focus:border-accent"
                />
              </label>
              <label className="flex flex-col gap-1">
                <span className="flex items-center gap-1 text-xs font-medium text-slate">
                  <Clock size={12} /> {T("timeLimitLabel")}
                </span>
                <input
                  type="number"
                  min={1}
          ...
          <div className="mt-block grid grid-cols-1 gap-stack xl:grid-cols-2 xl:items-start">
                ...
                <div className="flex items-start gap-tight">
                  <div className="flex flex-col items-center gap-1 pt-2 text-slate">
                    ...
                    <button
                      ...
                      disabled={qIndex === 0}
                      className="text-[10px] disabled:opacity-20"
                    >
                      ▲
                    </button>
                    <button
                    ...
                    {q.image_url ? (
                      <div className="relative mt-tight inline-block">
                        <img
                          src={q.image_url}
                          alt=""
                          className="h-24 w-auto rounded-control border border-line object-cover"
                        />
                        <button
                          type="button"
                      ...
                      {q.options.map((o, oIndex) => (
                        <div key={oIndex} className="flex items-center gap-1.5">
                          <button
                            ...
                            onClick={() =>
                              q.multi_select
                                ? toggleCorrectMulti(qIndex, oIndex)
                                : setCorrectSingle(qIndex, oIndex)
                            }
                          ...
                          {o.image_url ? (
                            ...
                            <label
                              className="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-control border border-dashed border-line text-slate hover:border-accent hover:text-accent"
                              title={T("addOptionImage")}
                            >
                              {uploadingKey === `${qIndex}-${oIndex}` ? (
                                <Loader2 size={13} className="animate-spin" />
                              ) : (
                                <ImageIcon size={13} />
                              )}
                              <input
                                ...
                                onChange={(e) => {
                                  const f = e.target.files?.[0];
                                  e.target.value = "";
                                  if (f) uploadImage(f, { qIndex, oIndex });
                                }}
                              />
                            </label>
                          )}
                    ...
                    <div className="mt-tight flex flex-wrap items-center gap-stack">
                      ...
                      <label className="flex items-center gap-1.5 text-xs text-slate">
                        <input
                          ...
                          onChange={(e) =>
                            updateQuestion(qIndex, { multi_select: e.target.checked })
                          }
            ...
            <button
              type="button"
              onClick={addQuestion}
              className="flex items-center justify-center gap-1.5 rounded-control border border-dashed border-line py-tight text-sm text-slate transition-colors duration-base hover:border-accent hover:text-accent xl:col-span-2"
            >
      ...
      {/* ---------- Footer ---------- */}
      <div className="flex flex-wrap items-center justify-between gap-tight border-t border-line px-block py-3">
