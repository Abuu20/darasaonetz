...
export default function CourseFormModal({
  ...
  const handleNext = async () => {
    ...
    if (STEP_IDS[step] === "curriculum" && savedCourse) {
      courseQueries
        ...
        .then(fresh => setSavedCourse(prev => (prev ? { ...prev, lessons: fresh.lessons } : fresh)))
        .catch(() => {});
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-night/80 px-gutter py-block" role="dialog" aria-modal="true">
      <ResizablePanel
        defaultWidth={typeof window !== "undefined" ? Math.min(920, window.innerWidth - 48) : 920}
        defaultHeight={typeof window !== "undefined" ? Math.min(760, window.innerHeight - 48) : 760}
        minWidth={480}
        minHeight={420}
