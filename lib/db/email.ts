...
const NEWSLETTER_TEMPLATE_ID = import.meta.env.VITE_EMAILJS_NEWSLETTER_TEMPLATE_ID as string | undefined;
...
export const emailService = {
  ...
  subscribeNewsletter: async (email: string) => {
    const sent = await sendViaEmailJS(NEWSLETTER_TEMPLATE_ID ?? "", { subscriber_email: email }).catch(() => false);
    await supabase.from("newsletter_subscribers").insert([{ email }]).then(({ error }) => {
      if (error) console.warn("[newsletter_subscribers insert] (non-fatal, table may not exist yet):", error.message);
    });
    if (!sent) console.info("[emailService] EmailJS not configured; subscriber saved to Supabase only.");
    return true;
  },
};
