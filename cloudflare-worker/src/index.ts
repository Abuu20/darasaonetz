...
export interface Env {
  ...
  BREVO_API_KEY: string; // from Brevo → Settings → SMTP & API → API Keys
  BREVO_LIST_ID: string; // numeric id of the Brevo contact list to subscribe people into
}
...
// ---- Newsletter: subscribe an email into Brevo (real campaign sending
// lives in Brevo's own dashboard — this Worker's only job is to add the
// contact server-side, so the Brevo API key never reaches the browser) ----

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

async function handleNewsletterSubscribe(request: Request, env: Env, origin: string): Promise<Response> {
  ...
  if (!email || !EMAIL_RE.test(email)) {
    return json({ error: "A valid email is required" }, 400, origin, env);
  }
  ...
  const res = await fetch("https://api.brevo.com/v3/contacts", {
    ...
    body: JSON.stringify({
      ...
      updateEnabled: true, // re-subscribing (or a contact Brevo already knew about) is a success, not an error
    }),
  });

  // Brevo returns 201 for a new contact, 204 for an existing one that was
  // updated — both mean "this person is now on the list."
  if (res.ok || res.status === 204) {
    return json({ subscribed: true }, 200, origin, env);
  }

  const detail = await res.text().catch(() => "");
  console.error("[newsletter/subscribe] Brevo error", res.status, detail);
  return json({ error: "Could not subscribe right now. Please try again shortly." }, 502, origin, env);
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "POST" && pathname === "/newsletter/subscribe") {
