...
export default function TeacherDashboard() {
  ...
  async function fetchCourses() {
    ...
    if (!error && data) {
      ...
      const totalStudents = data.reduce((acc, course) => acc + (course.enrollments?.length || 0), 0)
      const totalRevenue = data.reduce((acc, course) => acc + ((course.enrollments?.length || 0) * (course.price || 0)), 0)
                          ...
                          return (
                            <Card key={course.id} hover>
                              ...
                              <div className="space-y-2 text-sm">
                                ...
                                <p className="text-gray-500">Students: {course.enrollments?.length || 0}</p>
                        ...
                        {reviews.map(review => (
                          <Card key={review.id}>
                            <div className="flex items-start gap-4">
                              <Avatar src={review.profiles?.avatar_url} alt={review.profiles?.full_name} size="md" />
                              <div className="flex-1">
                                <div className="flex items-center justify-between flex-wrap gap-2">
                                  <div>
                                    <p className="font-semibold">{review.profiles?.full_name || 'Anonymous'}</p>
                                    <p className="text-sm text-gray-500">{review.courses?.title}</p>
                                  </div>
                                ...
                                {review.teacher_response ? (
                                  <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                                    <p className="text-sm font-medium text-blue-600 mb-1">Your Response:</p>
                                    <p className="text-sm text-gray-700">{review.teacher_response}</p>
                                    <button onClick={() => {
                                  ...
                                  <div className="mt-4">
                                    <textarea value={responseText} onChange={(e) => setResponseText(e.target.value)} rows="3" className="w-full px-3 py-2 border rounded-lg" placeholder="Write your response..." disabled={submitting} />
                                    <div className="flex gap-2 mt-2">
                                      <Button size="sm" onClick={() => handleRespondToReview(review.id, review.course_id, responseText)} disabled={submitting}>
                                  ...
                                  <button onClick={() => setRespondingTo(review.id)} className="mt-3 text-sm text-blue-600 hover:underline">
